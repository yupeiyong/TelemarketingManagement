...
namespace JJsites.WebPc.Lib.umeditor.net
{
    ...
    public class Uploader
    {
        ...
        */
        ...
        public Dictionary<string, string> upFile(HttpContext cxt, string basePath, string[] filetype, int size, int uploadSubFolderMaxCount)
        {
            //basePath = basePath + DateTime.Now.ToString("yyyy-MM-dd") + "/";
            //uploadpath = cxt.Server.MapPath(basePath);//获取文件上传路径
            ...
            try
            {
                ...
                if (state == "SUCCESS")
                {
                    ...
                    {
                        var server = AutofacContainerFactory.GetContainer().Resolve<FilePoolServer>();
                        filename = server.Put(stream, uploadFile.ContentLength, originalName).RawFilename;
                    }
                }
            }
            catch (Exception)
        ...
        public Dictionary<string, string> upScrawl(HttpContext cxt, string pathbase, string tmppath, string base64Data)
        {
        ...
        */
        ...
        private Dictionary<string, string> getUploadInfo()
        {
            ...
            infoList.Add("size", uploadFile.ContentLength.ToString());
