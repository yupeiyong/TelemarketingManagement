...
namespace JJsites.WebPc.Areas.AdminManager.Controllers
{
    ...
    public class LowerStatementReportController : AbstractBaseController
    {
        ...
        {
            var model = LowerStatementReportService.Search(dto);
            return PartialView(model);
        }


        public ViewResult Details(GetLowerStatementReportDto dto)
        {
            var model = new LowerStatementReportDetailsViewModel {Title = $"没有找到指定的报表（Id：{dto.Id}）！"};
            var report = model.LowerStatementReport = LowerStatementReportService.GetByParameters(dto);
            if (report != null)
        ...
        public PartialViewResult DetailsListPartial(GetLowerStatementReportDto dto)
        {
            var model = new LowerStatementReportDetailsViewModel {Title = $"没有找到指定的报表（Id：{dto.Id}）！"};
            var report = model.LowerStatementReport = LowerStatementReportService.GetByParameters(dto);
            if (report != null)
        ...
        //{
        //    LowerStatementReportService.GenerateReports(CurrentUser.Id);
